/*! SpringRoll PBSKIDS Container 0.2.0 */
/*! Bellhop 1.2.0 */
...
(function(){
  ...
  var _log=function(message,args,type,force){
        if(_DEBUG===true||force===true){
          if( typeof message == "string" || !!args ){ message = _APP_NAME + " ver. " + _APP_VERS + " | " + message;}
          else { args = message; message = _APP_NAME + " ver. " + _APP_VERS + " | "; }
          if(typeof console!=="undefined"){if(type=="error")if(console.log){console.log(message,args);return;}else if(window.Error)throw new Error(message);if(type=="info"&&console.info){console.info(message,args);return;}if(type=="warn"&&console.log){console.log(message,args);return;}if(console.log)console.log(message,args);}else if(window.debug)window.debug.log.apply(message,args);
        }
      };
  var _createPackage=function(packagePath){var dir,pak=typeof exports!=="undefined"?exports:window;if(typeof packagePath==="string"){dir=packagePath.split(".");for(var i in dir){if(!pak[dir[i]])pak[dir[i]]={};pak=pak[dir[i]];}}return pak;};
  var _getCookie=function(c_name){if(document.cookie.length>0){var c_end,c_start=document.cookie.indexOf(c_name+"=");if(c_start!=-1){c_start=c_start+c_name.length+1;c_end=document.cookie.indexOf(";",c_start);if(c_end==-1)c_end=document.cookie.length;return decodeURI(document.cookie.substring(c_start,c_end));}}return"";};
  ...
  (function( factory ){
    ...
    /************************************
     * KART KINGDOM . Avatar Animation
     * PBS KIDS Digital
     * July 2014
     *
     * @author Renzo M. Olguin
     ...
     ************************************/
      ...
      ***************************/
      (function(){
        ...
        if( isNaN( _options.scale ) ) _options.scale = "1.0";
        else _options.scale = Math.min( _options.scale , 2.34 );//DO NOT exceed a scale of 234%;

        if( (_options.scale + "").indexOf(".") == -1 ) _options.scale += ".0";
        _log( "scale ajusted: ", _options.scale);


        //Load the png data into an image object and build the canvas on which to draw the avatar
        var avatarEl = $("<canvas class='" + _options.avatarBaseClass + "' width='" + (350 * parseFloat(_options.scale)) + "px' height='" + (350 * parseFloat(_options.scale)) + "px'></canvas>").addClass( _options.avatarBaseClass ).appendTo( $(parentEl) );
        _avatarCanvas = avatarEl.get(0).getContext("2d");
      ...
      ***************************/
      ...
      function _animate( animation ){
        ...
        if( !_avatarCanvas || !_avatarPNG || !_avatarJSON ) return;
        ...
        for( var i in animationSequence[0] ){
          ...
          if( _avatarJSON[layerClass] ) _drawAvatarLayer( _avatarCanvas, _avatarPNG, transform, _options.scale, _avatarJSON[layerClass]);
        }
      }//_animate()
      ...
      {//Capture Avatar png sprite sheet, json descriptor for png sprite sheet, and animation file/data.
        ...
        ///_log( "_avatarJSONString: ", _avatarJSONString );
        _avatarJSONString = _avatarJSONString.replace(/("r[x|y]"):"null"/g, "$1:0");
        //_log( "_avatarJSONString fixed: ", _avatarJSONString );
        _avatarJSON = JSON.parse( _avatarJSONString );
        //_log("_avatarJSON: ", _avatarJSON);
      ...
      function _loadAvatarSprite( avatarLayers ){
        ...
        $.ajax({//load avatar sprite sheet and json descriptor
      ...
      function _buildRandomAvatar( groups ){
        ...
        function __onLoadGroup( groupName, groupID, response ){
          if( response && response.packet && response.packet.length > 0 ){
            if( !PBS.KIDS.KartKingdom.productGroups[ groupName ] ){
              ...
              if( supportsLocalStorage ) localStorage["/kartkingdom:productGroups"] = JSON.stringify( PBS.KIDS.KartKingdom.productGroups );
            }
        ...
        function __loadGroup( groupName, groupID ){
          $.ajax({url : _options.worldDomain + "/api/product_groups/" + groupID +"/products.nexus",
...
(function(){
  /* globals console, exports, PBS, namespace */
	/* jshint quotmark: false */
	"use strict";
	...
	var _CSS_VERS = "1.1";
	var _DEBUG = window.location.hostname.match(/^((?!www(\-tc)?\.).+)pbskids\.org$/) ? true : false; //only debug on subdomains of pbskids.org
  var _log=function(message,args,type,force){if(_DEBUG===true||force===true){if( typeof message == "string" || !!args ){ message = _APP_NAME + " ver. " + _APP_VERS + " | " + message;} else { args = message; message = _APP_NAME + " ver. " + _APP_VERS + " | "; } if(typeof console!=="undefined"){if(type=="error")if(console.log){console.log(message,args);return;}else if(window.Error)throw new Error(message);if(type=="info"&&console.info){console.info(message,args);return;}if(type=="warn"&&console.log){console.log(message,args);return;}if(console.log)console.log(message,args);}else if(window.debug)window.debug.log.apply(message,args); } };
	var _createPackage = function(packagePath) {var dir, pak = typeof exports !== "undefined" ? exports : window; if (typeof packagePath === "string") {dir = packagePath.split("."); for (var i in dir) {if (!pak[dir[i]]) pak[dir[i]] = {}; pak = pak[dir[i]]; } } return pak; };
  var _getCookie=function(c_name){if(document.cookie.length>0){var c_end,c_start=document.cookie.indexOf(c_name+"=");if(c_start!=-1){c_start=c_start+c_name.length+1;c_end=document.cookie.indexOf(";",c_start);if(c_end==-1)c_end=document.cookie.length;return decodeURI(document.cookie.substring(c_start,c_end));}}return"";};
	...
	(function(factory){
		// create top-level object
		_createPackage("PBS.KIDS.KartKingdom");
		if( typeof(namespace) == "function" ) namespace("springroll.pbskids.kartkingdom");
		...
		//Assign to namespace
		if( typeof(namespace) == "function" ) namespace("springroll.pbskids.kartkingdom").MinigameEvent = PBS.KIDS.KartKingdom.minigameEvent;
		...
		PBS.KIDS.KartKingdom.minigame = function( gameID, elementID, pauseFunc, unPauseFunc, options ){
			...
			var _that = this; //May not need to capture this
			...
			var _AVATAR_LIB = _PBS_DOMAIN + "/kartkingdom/api/kartkingdom.avatar." + (_DEBUG && !window.location.hostname.match(/(soup|ernie|bert).pbskids.org/) ? "latest.js" : "min-latest.js");
			...
			_options.enable_notifications = (_options.enable_notifications !== false);
			_notifications_enabled = _options.enable_notifications;
      ...
      function _finalize(){
				...
				if (!_gameWrapper.css("position") || _gameWrapper.css("position") == "static") _gameWrapper.css("position", "relative");
        $(_gameWrapper).css({"overflow":"hidden"});

				// If no jquery on jquery then add local version to target
				if (!target.$) target.$ = $;
          ...
          success: function(response){
            ...
            for (var f in response.frames ){
              styles += rewardClassName + '.' + f.replace(".png","").replace(spriteQuality + "_","") + ' .kartkingdom-reward-icon {' +
						...
						$("<style/>").attr("type", "text/css").appendTo("head").html(styles);
					}
				});
			...
			function _destroy(){
				...
				// [TODO] remove elements/markup
				// [TODO] remove css styles written to document
				// [TODO] abort any ajax requests
			} //_destroy()
      ...
      function _removeEventListener(type, listener){
      	if(_gameWrapper) $(_gameWrapper).off(type, listener);
      }
      ...
      function _addEventListener(type, listener){
        ...
        if( type == PBS.KIDS.KartKingdom.minigameEvent.MINIGAME_READY && _apiReady ){
					...
					for (var i in payload) e[i] = payload[i];
      ...
      function _dispatchEvent(type, payload){
        ...
        switch( type ){
					...
					case PBS.KIDS.KartKingdom.minigameEvent.POWERUP_CONSUME_FAILED:
						...
						for (var i in payload) e[i] = payload[i]; //add custom data to event object
      ...
      var _onResize = function(){
    	...
    	function _checkGameWidth(wrapper, breakpoints){
				$(wrapper).removeClass(); //remove all classes
      ...
      function _onReadyPayload(){
								...
								$.ajax({
									...
									success: function(response){
										for (var i in _player.powerups){
											...
											if (response[i]) _player.powerups[i].quantity += response[i];
			...
			function _userValidated(user, avatar, powerups){
				...
				// Load Avatar Script for building avatar at end of mini-game
        if( _player.avatar && !PBS.KIDS.KartKingdom.avatar ) $.ajax({ url: _AVATAR_LIB });
			...
			function _validateKartKingdomAccess(checkVirtualWorld){
				...
				if (checkVirtualWorld){
					...
					var __channelReadyTimeout;
					var __onChannelReady = function(){
						clearTimeout(__channelReadyTimeout);
					...
					if (!_vwChannel){
						...
						__channelReadyTimeout = setTimeout(__onChannelReady, __timeoutDelay);
					}
					else{
						__onChannelReady();
					}
					...
					$.ajax({
						...
						success: function(response){
							...
							if (response.packet && response.packet.user && response.packet.user.id){
								...
								_log("ValidateKartKingdomAccess :: User is logged in, but does not have virtu world access.", response);
								_complete(); //done
							}
						}
					});
			...
			function _loadDependencies(onCompleteCallback){
				...
				var __checkLoading = function(dep){
					switch ((dep.type || "").toLowerCase()){
						case "js":
						case "javascript":
							if (window[dep.name]) __onLoaded(dep, window[dep.name]);
							else window.setTimeout( function(){ __checkLoading(dep); }, 20);
							break;
			...
			function _showRewardNotification(reward, count, message, eventID){
				if (!_notifications_enabled) return;
					...
					$("#kartkingdom-reward-notification dl")
						...
						.animate(__animateProps, {
							...
							"complete": function(){
								...
								if (_options.hide_notification_delay > 0) _hideNotificationTimeout = setTimeout(_hideRewardNotification, _options.hide_notification_delay);
							}
						});
        ...
        _checkGameWidth(
			...
			function _levelComplete(){
				...
				if ($("#kartkingdom-level-complete-screen-wrapper").get(0)) return;
	            ...
	            $(_carouselList).animate({ "left" : posLeft }, rotationSpeed);
					...
					if( PBS.KIDS.KartKingdom.avatar && _player.avatar ){
						PBS.KIDS.KartKingdom.avatar( $(".vw-background", dialog), {
		...
		//Assign to namespace
		if( typeof(namespace) == "function" ) namespace("springroll.pbskids.kartkingdom").Minigame = PBS.KIDS.KartKingdom.minigame;
...
/*! SpringRoll PBSKIDS Container 0.2.0 */
!function(){"use strict";!function(a){var b,c=include("localStorage"),d=include("jQuery"),e=function(c,d,e){b===a&&(b=include("springroll.pbskids.UUID")),this.onlineStatus=!0,this.timer=null,this.currentBatch=[],this.inTransitKeys={},this.keyPrefix=d,this.batchSize=c,this.eventServiceUrl=e,this.resetKey()},f=e.prototype,g="bufferQueue";f.processBatch=function(b){var e=this.inTransitKeys;return e[b]===a?(e[b]=0,d.ajax({url:this.eventServiceUrl,data:c.getItem(b),type:"POST",contentType:"application/json",dataType:"json",success:function(){h(b),c.removeItem(b),delete e[b]},error:function(a){400==a.status&&(h(b),c.removeItem(b)),delete e[b]}}),!0):!1};var h=function(a){var b=j(),d=b.indexOf(a);d>-1&&(b.splice(d,1),c.setItem(g,JSON.stringify(b)))},i=function(a){var b=j(),d=b.indexOf(a);-1===d&&(b.push(a),c.setItem(g,JSON.stringify(b)))},j=function(){try{return JSON.parse(c.getItem(g))||[]}catch(a){}return[]};f.startTimer=function(){this.stopTimer(),this.timer=setInterval(this._onTimerTick.bind(this),1e3)},f._onTimerTick=function(){if(this.onlineStatus)for(var a=j(),b=0;b<a.length&&!this.processBatch(a[b]);b++);},f.stopTimer=function(){clearInterval(this.timer),this.timer=null},f.enable=function(){this.inTransitKeys={},this.currentBatch.length=0,this.resetKey(),this.startTimer()},f.disable=function(){this.stopTimer()},f.setAutoFlush=function(a){a?this.timer||this.startTimer():this.stopTimer()},f.setOnline=function(a){this.onlineStatus=a},f.pushEvent=function(a){this.inTransitKeys[this.bufferKey]=0,this.currentBatch.push(a),c.setItem(this.bufferKey,JSON.stringify(this.currentBatch)),i(this.bufferKey),this.currentBatch.length>=this.batchSize&&(this.currentBatch.length=0,delete this.inTransitKeys[this.bufferKey],this.resetKey())},f.flushAll=function(){if(this.currentBatch.length=0,delete this.inTransitKeys[this.bufferKey],this.resetKey(),!this.timer)for(var a=j(),b=0;b<a.length;b++)this.processBatch(a[b])},f.resetKey=function(){this.bufferKey=this.keyPrefix+b.genV4().hexString},f.end=function(){this.currentBatch.length=0,delete this.inTransitKeys[this.bufferKey],this.resetKey()},f.destroy=function(){this.end(),this.currentBatch=null,this.inTransitKeys=null,this.stopTimer()},namespace("springroll.pbskids").BufferedQueue=e}(),function(){var a,b,c,d,e=function(e){void 0===a&&(a=include("springroll.pbskids.UUID"),b=include("springroll.pbskids.BufferedQueue"),c=include("springroll.pbskids.Platform"),d=include("PBS.KIDS.identity",!1)),this.options=Object.merge({resource:"game",domain:"http://progresstracker.pbskids.org:8000"},e||{});var j=g.v1,k=this.options.resource;this.eventKeys=i.v1,k&&g.v2[k]&&(j=g.v2[k],this.eventKeys=i.v2[k]),this.queue=new b(h,f,this.options.domain+j),this.platform=new c,this.sessionId=null,this._timer=null,this._enabled=!1,this.resetSessionId(),this.enabled=!1},f="PBS_event_",g={v2:{video:"/progresstracker/api/v2/videos/events.json",game:"/progresstracker/api/v2/games/events.json"},v1:"/progresstracker/api/v1/rawevents.json"},h=5,i={v2:{game:["timestamp","user_ids","game_id","device_id","platform_id","event_id","event_data","game_session"],video:["timestamp","user_ids","video_id","device_id","platform_id","event_id","event_data","video_session"]},v1:["timestamp","user_ids","game_id","device_id","platform_id","event_id","event_data","game_session"]},j=e.prototype;j.resetSessionId=function(){this.sessionId=a.genV4().hexString+a.genV4().hexString},j.end=function(){this.queue.end()},j.setAutoFlush=function(a){this.queue.setAutoFlush(a)},j.checkStatus=function(){this.queue.setOnline(navigator.onLine)},Object.defineProperty(j,"enabled",{get:function(){return this._enabled},set:function(a){this._enabled=a,this._timer&&(clearInterval(this._timer),this._timer=null),a?(this._timer=setInterval(this.checkStatus.bind(this),1e4),this.queue.enable()):this.queue.disable()}}),j.flushAll=function(){this.queue.flushAll()},j.appendSessionId=function(a){return a.game_session=this.sessionId,this},j.appendPlatformDetails=function(a){return a.platform_id=this.platform.browser,a.device_id=this.platform.OS,this},j.appendTimeStamp=function(a){var b=new Date;return a.timestamp=b.getTime(),this},j.pushEvent=function(a){if(!this._enabled)return this;if(void 0===a.user_ids&&(a.user_ids=[]),a.is_logged_in=!1,d)for(var b=d.getCurrentUsers(),c=0;c<b.length;c++)a.user_ids[c]=b[c].userid,a.is_logged_in=b[c].isloggedin;return this.appendTimeStamp(a).appendSessionId(a).appendPlatformDetails(a),this.validateEvent(a)&&this.queue.pushEvent(a),a},j.validateEvent=function(a){for(var b=0;b<this.eventKeys.length;b++)if(!k(a[this.eventKeys[b]]))return!1;return!0};var k=function(a){return!(null===a||"undefined"==a||""===a)};j.destroy=function(){this.enabled=!1,this.queue.destroy(),this.queue=null,this.platform=null},namespace("springroll.pbskids").LearningAnalytics=e}(),function(){var a=new springroll.ContainerPlugin;a.setup=function(){var a=include("springroll.pbskids.LearningAnalytics");this.learning=new a(this.options.analytics)},a.open=function(){this.learning.enabled=this.isProduction;var a=b.bind(this);this.client.on({learningEvent:a,progressEvent:a})};var b=function(a){var b=a.data;this.loggingService.send("pt-event",b),this.learning.pushEvent(b),this.trigger("learningEvent",b)};a.close=function(){this.learning&&(this.learning.enabled=!1,this.client.off("learningEvent"),this.client.off("progressEvent"))},a.teardown=function(){this.learning&&(this.learning.destroy(),delete this.learning)}}(),function(){var a=function(){this.browser=this.searchString(c)||"unknown browser",this.version=this.searchVersion(navigator.userAgent)||this.searchVersion(navigator.appVersion)||"unknown version",this.versionSearchString=null,this.OS=this.searchString(d)||"unknown OS"},b=a.prototype;b.searchString=function(a){for(var b=0;b<a.length;b++){var c=a[b].string,d=a[b].prop;if(this.versionSearchString=a[b].versionSearch||a[b].identity,c){if(-1!=c.indexOf(a[b].subString))return a[b].identity}else if(d)return a[b].identity}},b.searchVersion=function(a){var b=a.indexOf(this.versionSearchString);if(-1!=b)return parseFloat(a.substring(b+this.versionSearchString.length+1))};var c=[{string:navigator.userAgent,subString:"Chrome",identity:"Chrome"},{string:navigator.userAgent,subString:"OmniWeb",versionSearch:"OmniWeb/",identity:"OmniWeb"},{string:navigator.vendor,subString:"Apple",identity:"Safari",versionSearch:"Version"},{prop:window.opera,identity:"Opera",versionSearch:"Version"},{string:navigator.vendor,subString:"iCab",identity:"iCab"},{string:navigator.vendor,subString:"KDE",identity:"Konqueror"},{string:navigator.userAgent,subString:"Firefox",identity:"Firefox"},{string:navigator.vendor,subString:"Camino",identity:"Camino"},{string:navigator.userAgent,subString:"Netscape",identity:"Netscape"},{string:navigator.userAgent,subString:"MSIE",identity:"Explorer",versionSearch:"MSIE"},{string:navigator.userAgent,subString:"Gecko",identity:"Mozilla",versionSearch:"rv"},{string:navigator.userAgent,subString:"Mozilla",identity:"Netscape",versionSearch:"Mozilla"}],d=[{string:navigator.platform,subString:"Win",identity:"WindowsPC"},{string:navigator.platform,subString:"Mac",identity:"Mac"},{string:navigator.userAgent,subString:"iPhone",identity:"iPhone/iPod"},{string:navigator.userAgent,subString:"iPad",identity:"iPad"},{string:navigator.userAgent,subString:"Android",identity:"Android"},{string:navigator.platform,subString:"Linux",identity:"LinuxPC"}];namespace("springroll.pbskids").Platform=a}(),function(){var a=function(){};a.generate=function(){var b=a._getRandomInt,c=a._hexAligner;return c(b(32),8)+"-"+c(b(16),4)+"-"+c(16384|b(12),4)+"-"+c(32768|b(14),4)+"-"+c(b(48),12)},a._getRandomInt=function(a){return 0>a?NaN:30>=a?0|Math.random()*(1<<a):53>=a?(0|Math.random()*(1<<30))+(0|Math.random()*(1<<a-30))*(1<<30):NaN},a._getIntAligner=function(a){return function(b,c){for(var d=b.toString(a),e=c-d.length,f="0";e>0;e>>>=1,f+=f)1&e&&(d=f+d);return d}},a._hexAligner=a._getIntAligner(16),a.FIELD_NAMES=["timeLow","timeMid","timeHiAndVersion","clockSeqHiAndReserved","clockSeqLow","node"],a.FIELD_SIZES=[32,16,16,8,8,48],a.genV4=function(){var b=a._getRandomInt;return(new a)._init(b(32),b(16),16384|b(12),128|b(6),b(8),b(48))},a.parse=function(b){var c,d=/^\s*(urn:uuid:|\{)?([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{2})([0-9a-f]{2})-([0-9a-f]{12})(\})?\s*$/i;if(c=d.exec(b)){var e=c[1]||"",f=c[8]||"";if(e+f===""||"{"===e&&"}"===f||"urn:uuid:"===e.toLowerCase()&&""===f)return(new a)._init(parseInt(c[2],16),parseInt(c[3],16),parseInt(c[4],16),parseInt(c[5],16),parseInt(c[6],16),parseInt(c[7],16))}return null},a.prototype._init=function(){var b=a.FIELD_NAMES,c=a.FIELD_SIZES,d=a._binAligner,e=a._hexAligner;this.intFields=new Array(6),this.bitFields=new Array(6),this.hexFields=new Array(6);for(var f=0;6>f;f++){var g=parseInt(arguments[f]||0);this.intFields[f]=this.intFields[b[f]]=g,this.bitFields[f]=this.bitFields[b[f]]=d(g,c[f]),this.hexFields[f]=this.hexFields[b[f]]=e(g,c[f]/4)}return this.version=this.intFields.timeHiAndVersion>>12&15,this.bitString=this.bitFields.join(""),this.hexString=this.hexFields[0]+"-"+this.hexFields[1]+"-"+this.hexFields[2]+"-"+this.hexFields[3]+this.hexFields[4]+"-"+this.hexFields[5],this.urn="urn:uuid:"+this.hexString,this},a._binAligner=a._getIntAligner(2),a.prototype.toString=function(){return this.hexString},a.prototype.equals=function(b){if(!(b instanceof a))return!1;for(var c=0;6>c;c++)if(this.intFields[c]!==b.intFields[c])return!1;return!0},a.genV1=function(){var b=(new Date).getTime(),c=a._state;b!=c.timestamp?(b<c.timestamp&&c.sequence++,c.timestamp=b,c.tick=a._getRandomInt(4)):Math.random()<a._tsRatio&&c.tick<9984?c.tick+=1+a._getRandomInt(4):c.sequence++;var d=a._getTimeFieldValues(c.timestamp),e=d.low+c.tick,f=4095&d.hi|4096;c.sequence&=16383;var g=c.sequence>>>8|128,h=255&c.sequence;return(new a)._init(e,d.mid,f,g,h,c.node)},a.resetState=function(){a._state=new a._state.constructor},a._tsRatio=.25,a._state=new function(){var b=a._getRandomInt;this.timestamp=0,this.sequence=b(14),this.node=1099511627776*(1|b(8))+b(40),this.tick=b(4)},a._getTimeFieldValues=function(a){var b=a-Date.UTC(1582,9,15),c=b/4294967296*1e4&268435455;return{low:1e4*(268435455&b)%4294967296,mid:65535&c,hi:c>>>16,timestamp:b}},a.makeBackwardCompatible=function(){var b=a.generate;a.generate=function(c){return c&&1==c.version?a.genV1().hexString:b.call(a)},a.makeBackwardCompatible=function(){}},namespace("springroll.pbskids").UUID=a}(),function(){var a=function(a,b){this.socket=null,this.connected=!1,this.connecting=!1,this.socketBuffer=[],this._channel=null,a&&this.connect(a,b)},b=a.prototype;b.connect=function(a,b){if(("WebSocket"in window||"MozWebSocket"in window)&&a){b=b||1025,window.WebSocket=WebSocket||MozWebSocket,onCloseRemote=this._onCloseRemote.bind(this),onOpenRemote=this._onOpenRemote.bind(this),onCloseRemote(),this.connecting=!0;try{var c=this.socket=new WebSocket("ws://"+a+":"+b);return c.onopen=onOpenRemote,c.onclose=onCloseRemote,c.onerror=onCloseRemote,!0}catch(d){return onCloseRemote(),!1}}return!1},b._onOpenRemote=function(){this.connecting=!1,this.connected=!0;for(var a=0;a<this.socketBuffer.length;a++)this.socket.send(this.socketBuffer[a]);this.socketBuffer.length=0},b._onCloseRemote=function(){this.connecting=this.connected=!1,this.socketBuffer&&(this.socketBuffer.length=0),this.socket&&(this.socket.close(),this.socket.onopen=null,this.socket.onmessage=null,this.socket.onclose=null,this.socket.onerror=null,this.socket=null)},Object.defineProperty(b,"channel",{set:function(a){this._channel=a,this.send("session")},get:function(){return this._channel}}),b.send=function(a,b){var c={type:a,channel:this._channel};b&&(c.data=b),c=JSON.stringify(c),this.connecting?this.socketBuffer.push(c):this.connected&&this.socket.send(c)},b.destroy=function(){this._onCloseRemote(),this.socketBuffer=null},namespace("springroll.pbskids").LoggingService=a}(),function(){var a=new springroll.ContainerPlugin(20);a.setup=function(){var a=include("springroll.pbskids.LoggingService");this.loggingService=new a(this.options.remoteHost,this.options.remotePort)},a.teardown=function(){this.loggingService.destroy(),delete this.loggingService}}(),function(){var a=include("springroll.pbskids.kartkingdom.Minigame"),b=include("springroll.pbskids.kartkingdom.MinigameEvent"),c=new springroll.ContainerPlugin;c.setup=function(){this.minigame=null},c.open=function(){this.client.on("kk-init",d.bind(this))};var d=function(b){var c=b.data;this.dom.parentNode.id||(this.dom.parentNode.id=this.dom.parentNode.id+"-kartkingdom-wrapper"),this.minigame=new a(c.gameID,this.dom.parentNode.id,function(){this.paused=!0},function(){this.paused=!1},c.options),e(this.minigame,this.client),this.client.send("kk-init-done")},e=function(a,c){c.on("kk-event",function(b){a.event(b.data)}),c.on("kk-level-complete",function(b){a.levelComplete()}),c.respond("kk-has-power-up",a.hasPowerUp(event.data)),c.on("kk-use-power-ups",function(b){a.usePowerUps(b.data.powerupsObject,function(){c.send(b.data.successEvent)},function(){c.send(b.data.failureEvent)})}),a.addEventListener(b.MINIGAME_READY,function(a){c.send("kk-ready",a)}),a.addEventListener(b.LEVEL_COMPLETE_OPEN,function(a){c.send("kk-level-open",a)}),a.addEventListener(b.LEVEL_COMPLETE_CLOSED,function(a){c.send("kk-level-closed",a)}),a.addEventListener(b.LEVEL_RESET,function(a){c.send("kk-level-reset",a)}),a.addEventListener(b.RESOURCES_EARNED,function(a){c.send("kk-resources-earned",a)}),a.addEventListener(b.RESOURCE_REQUEST_COMPLETE,function(a){c.send("kk-resources-complete",a)})};c.teardown=function(){delete this.minigame}}(),function(){var a=new springroll.ContainerPlugin;a.open=function(){var a=b.bind(this);this.client.on({analyticEvent:a,trackEvent:a})},a.close=function(){this.client.off("analyticEvent"),this.client.off("trackEvent")};var b=function(a){var b=a.data;this.loggingService.send("ga-event",b);var c=include("GA_obj",!1);c&&this.isProduction&&c.trackEvent(b.category,b.action,b.label,b.value),this.trigger("analyticEvent",b)}}(),function(){var a=include("PBS.KIDS.$_auth",!1);a||(a=include("jQuery"));var b=new springroll.ContainerPlugin(10);b.setup=function(){this.isProduction=!1,this.apiDomain=this.options.apiDomain||"http://springroll.pbskids.org",this.hasHeadband=!!document.getElementById("headband-container");var b=include("org.pbskids.parentsBar",!1);this.headbandReady=!(!b||!b.ready),this.open=function(b,d){if(this.hasHeadband&&!this.headbandReady)return void a(document).one("org_pbskids_parentsbar_HeadbandEvent_HeadbandReady",function(){this._destroyed||(this.headbandReady=!0,this.open(b,d))}.bind(this));if("object"==typeof b&&(d=b,b=null),b=b||this.main.data("game"),!b)throw"No data-game attribute or game parameter";var e,f="/api/release/"+b,g=include("springroll.env",!1),h=this.main.data("domain");this.main.data("env")?a.getJSON(this.main.data("env"),function(a){this._destroyed||(e=h||a.domain||this.apiDomain,this.isProduction="prod"==a.status,this.openRemote(e+f+c(a,this.main),d))}.bind(this)):g?(e=h||g.domain||this.apiDomain,this.isProduction="prod"==g.status,this.openRemote(e+f+c(g,this.main),d)):(e=h||this.apiDomain,this.isProduction=!0,this.openRemote(e+f+c({},this.main),d))}};var c=function(a,b){a=a||{};var c=!!b.data("debug")||a.debug,d=b.data("status")||a.status,e=b.data("token")||a.token,f=[];return e&&f.push("token="+e),d&&f.push("status="+d),c&&f.push("debug=1"),f.length?"?"+f.join("&"):""}}();}();