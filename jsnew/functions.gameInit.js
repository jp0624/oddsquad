...
function setupGameContainer(){
	console.log('awardsLoaded: ', awardsLoaded);
	console.log('useragentInfoLoaded: ', useragentInfoLoaded);
	console.log('springrollLoaded: ', springrollLoaded);
	...
	if(awardsLoaded === true && useragentInfoLoaded === true && springrollLoaded === true){
		...
		console.log('gameInit(202) gameContainerSetup: ', gameContainerSetup);
...
function initGame(container, containerId){
	...
	console.log('gameInit(215) container: ', container);

	container.on('open', function() {
		
		console.log('gameInit(220) container.on(open)');
		...
		bellhop.on('add-award', function(event){
			...
			console.log('Add award triggered from bellhop: ', event.data);
			var id = event.data;
		...
		bellhop.on('sendGAEvent', function(event){
		//$(document).on('sendGAEvent', function(event){

			var gaData = event.data;
			
			var category 	= gaData.Category || 'none';
			var action 		= gaData.Action || 'none';
			var label 		= gaData.Label || 'none';
			var value 		= gaData.Value || 'none';

			console.log('Send GA Event Received');
			console.log('GA Event:' + ' Category: ' + category + ' Action: ' + action + ' Label: ' + label + ' Value: ' + value);

			//if(isEmpty(gaData.Label)) {
			if(typeof(gaData.Label) !== 'undefinded') {
				console.log('track 1');
			   GA_obj.trackEvent(String(gaData.Category), String(gaData.Action));
			//} else if (isEmpty(gaData.Value)) {
			} else if (typeof(gaData.Value) !== 'undefinded') {
				console.log('track 2');
			   GA_obj.trackEvent(String(gaData.Category), String(gaData.Action), String(gaData.Label));
			} else {
				console.log('track 3');
			   GA_obj.trackEvent(String(gaData.Category), String(gaData.Action), String(gaData.Label), String(gaData.Value));
			}
		...
		function _setValueError (result) {
			console.log("* Error setting value in DB: " + result);
		}
		...
		function setInDB (event){
			...
			try {
				...
				console.log("error setting value to db: " + e);
			}
		}
	...
	console.log('gameInit(377) container.open()');
	container.open();
}
