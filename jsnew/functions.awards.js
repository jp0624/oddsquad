...
function updateAwards() {
	...
	achievements = {
		...
		get: function(doneFunction) {
			...
			if (typeof window.org.pbskids.login === 'undefined' || !window.org.pbskids.login.loggedin || window.org.pbskids.login.loggedin == null || window.org.pbskids.login.loggedin == 'undefined') { 
				window.org.pbskids.login
				...
				$.ajax({
		        	...
		        	async: false,
					dataType: 'html',
					url: "/oddsquad/php/getAchievements.php",
					data: {
						//username:window.org.pbskids.login.user.name,
						//userid:window.org.pbskids.login.user.name
						username:achievements.getCookie("pbskids.username"),
						userid:achievements.getCookie("pbskids.userid")
					},
					...
					success: function(data) {
						...
						console.log('AWARDS UNLOCKED: ', achievements.unlocked);
				...
				/*
				$.post("/oddsquad/php/getAchievements.php", { username:window.org.pbskids.login.user.name, userid:window.org.pbskids.login.user.name} ).done(function(data) {
...
$(document).ready( function() {
	...
	$(document).on('AWARDS_CHECKED', function() {
		...
		console.log('awardsChecked: ', awardsChecked);
		console.log('awardsCreated: ', awardsCreated);
		...
		console.log('skipped award page update')
