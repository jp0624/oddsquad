...
function attrSettings(elem, _attr) {
...
function changePage(target, doneFunction){
	...
	console.warn('params.status: ', params.status);
	...
	if(params.status && newPage >= 0) {
		...
		console.warn('Im here 1');
		gotoStage(newPage, 50)
		
		params.status = false;
		
	} else if(newPage >= 0){
		console.warn('Im here 2');
	...
	if(target.length > 1) {
		if(doneFunction){
			console.warn(' :) CHANGE PAGE FIRED WITH DONE FUNCTION');
			initOverlay(target, doneFunction);
		}else {
			console.warn(' :( CHANGE PAGE FIRED WITH NO DONE FUNCTION');
			initOverlay(target);
		};
	} else {
		if(doneFunction){
