...
$(document).on ({
	"PBS_BAR_LOADED" : function ( event ) {
		...
		if ( pbsBarLoaded && doorsCreated) {
			//update hall of doors based on date
			updateDoors();
		}
	...
	if($('[data-pageDepth="root"]').exists() == true) {
		...
		setTimeout(function () {
				...
				if(params !== undefined) {
					...
					console.log('params[1].dir: ', params[1].dir);
...
function buildContent(num) {
	...
	if($('.award-lst').exists() || $('.agents-lst').exists() || $('.agent-dashboard').exists()){
		...
		.complete( function() {
			...
			console.table(site.awards);
			
		});
	};
	
	if($('.wallpapers-lst').exists()){
		$.getJSON('/oddsquad/json/gallery.json', function(data) {
			//console.log('AWARDS JSON GET STARTED');
			site.gallery = data.gallery;
		})
		.success( function() {
			//console.log('GALLERY JSON GET SUCCESS');
			buildGallery(0);
		})
		.error( function(jqXHR, textStatus, errorThrown) {
			console.log('getJSON error: ', textStatus);
		...
		.complete( function() {
			//console.log('GALLERY JSON GET COMPLETE');
...
function updateAwards() {
	...
	if (!userLogin[0].isloggedin || $('body').attr('data-agent-login') == 'false' || achievements.unlocked[0] == '' || achievements.unlocked[0] == null || achievements.unlocked[0] == undefined || achievements.unlocked.length == 0 || achievements.unlocked == null || achievements.unlocked == 'undefined' || achievements.unlocked == '') {
	...
	if(userLogin[0].isloggedin && achievements.viewed.length > 0 && achievements.viewed[0] !== '') {
...
function updateDoors() {
	...
	for(var i=0; i < site.doors.length; i++) {
		if((site.doors[i].year <= year && site.doors[i].month <= month && site.doors[i].day <= day) || (site.doors[i].year <= year && site.doors[i].month < month) || (site.doors[i].year < year)) {
			$('[data-door="' + (i+1) + '"]').attr({
				'data-status': 'viewed'
			});
			doorsUnlocked++;
	...
	console.log('doorsUnlocked: ', doorsUnlocked);
	...
	$('.doors-lst li').on('tap', function() {
		...
		if ($(this).attr('data-status') == 'viewed') {
			$('body').fadeOut(250);
			setTimeout(function() {
				window.location = '/oddsquad/agents/hallofdoors/' + door + '/index.html'
			 }, 250);
		}
	});
...
$(document).on('click', '.set-background', function(event) {
	event.preventDefault();
	
	var bgImg = $(this).attr('data-bgnum');
	//console.log('change bg to: ', bgImg);
...
function updateAgentSections() {
	...
	//update doors unlocked in agent dashboard
	var d = new Date();
	var month = d.getMonth()+1,
		day = d.getDate(),
		year = d.getFullYear();
...
$(window).resize( function () {
	...
	if($('[data-overlay="awards"]').exists() == true || $('[data-overlay="hod"]').exists() == true) {
		location.reload();
		/*
