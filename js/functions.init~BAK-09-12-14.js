...
function buildAgents() {
	...
	console.log('TOT NUM OF AGENTS: ', agentsTotal);
		...
		for (i = 0; i < agentsTotal; i++) {
			...
			$(container).find('ul.agents-lst.disp-mobile').append('<li class="agent-' + (i + 1) +'" data-agentid="' + i + '"><figure><img src="/oddsquad/img/agents/' + site.agents[i].img1 + '" /></figure><dl><dt class="name">' + site.agents[i].name + '</dt><dt>Department</dt><dd>' + site.agents[i].department + '</dd><dt>Partner</dt><dd>' + site.agents[i].partner + '</dd><dt>Accessory</dt><dd>' + site.agents[i].accessory + '</dd><dt>Status</dt><dd>' + site.agents[i].status + '</dd></dl></li>');
...
function updateAwards() {
	...
	$('.award-lst li').on('tap', function() {
		...
		var award = parseInt($(this).attr('data-award'));
		...
		if($(this).attr('data-status') == 'unlocked') {
			...
			achievements.add(award, achievements.VIEWED);
...
function updateDoors() {
	...
	if(locker.viewed.length > 0 && locker.viewed[0] !== '') {
		...
		for(var i=0; i < locker.viewed.length; i++) {
			console.log('locker viewed: ', locker.viewed[i]);
